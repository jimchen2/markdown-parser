...
const DocumentTree: React.FC<DocumentTreeProps> = React.memo(
  ({ mobile, newlines = 4, fontSize = "text-2xl", onSelectDocumentId }) => {
    ...
    useEffect(() => {
      const fetchDocumentTrees = async () => {
        try {
          ...
          const documentTypes: DocumentType[] = Object.entries(groupedDocuments).map(
